...
private struct ReviewMediaStrip: View {
    ...
    /// Whether this day's cached collage predates the current design (#160).
    ///
    /// Held in state and read on appear rather than checked in `body`, because
    /// it opens a file and `body` runs on every redraw. The export screen took
    /// this shape for the same reason (#247); the first version of this row did
    /// not, which is a disk read per redraw for an answer that changes only
    /// when the day is regenerated.
    @State private var collageIsStale = false
    ...
    /// Re-read the collage's design stamp. Cheap, and only on the two events
    /// that can change the answer: arriving at the day, and finishing a
    /// regeneration of it.
    private func refreshCollageStaleness() {
        guard let (_, layoutURL) = collageInfo else {
            collageIsStale = false
            return
        }
        collageIsStale = LayoutSidecar.read(at: layoutURL).isStale
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Spacing.sm) {
            ...
            if let (url, layoutURL) = collageInfo {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    // A collage cached before the current design was rendered
                    // by the old look, and it will keep rendering it until this
                    // day is regenerated by hand. Nothing surfaced that, so a
                    // redesign left old collages in place indefinitely (#160).
                    if collageIsStale, let onRegenerate {
                        BrandBanner(
                            icon: "clock.arrow.circlepath",
                            message: "This collage was made with an older version of the "
                                   + "design, so it does not match the current one. "
                                   + "Regenerate the day to rebuild it.",
                            style: .warning,
                            actions: [BrandBannerAction(label: "Regenerate collage",
                                                        action: onRegenerate)]
                        )
                        .padding(.horizontal, Spacing.xl)
                    }
        ...
        .padding(.bottom, Spacing.xs)
        .onAppear { refreshCollageStaleness() }
        // A finished regeneration rewrites the sidecar with the current design,
        // so the badge has to go without leaving the screen. graphicVersion is
        // bumped by every completed render of this day.
        .onChange(of: graphicVersion) { _, _ in refreshCollageStaleness() }
        .onChange(of: isRegenerating) { _, running in
            if !running { refreshCollageStaleness() }
        }
    }
}

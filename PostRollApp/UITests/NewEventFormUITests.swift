...
/// Whether Return commits the form, and only when Dan opened it (#883, #848, #844).
///
/// Both halves of one rule, in one launch. Step 3 of `docs/HAND-CHECK.md`
/// pressed this by hand, on the recorded grounds that XCUITest never made the
/// New Event form readable on the runner (#860, and the comment on #848). That
/// was measured before #877 found what the harness does with text: the words
/// arrive as an element's VALUE with an EMPTY label, so a check reading the
/// label reports them absent while they are plainly on screen. That is
/// indistinguishable from a window the harness cannot see into, and it is the
/// mistake both of this file's first dispatches shipped with.
///
/// The outcome is read from the store on disk rather than from the screen. A
/// sheet that closes having created nothing looks exactly like one that closed
/// having created something, and what #848 and #844 are about is whether an
/// event was MADE.
///
/// ── why one launch rather than two (#896) ────────────────────────────────────
///
/// These were two classes with identical launch conditions, and a launch costs
/// 42 to 55 seconds on the runner: measured 47.5 and 53.0 in run 32684959882,
/// in a job of 342 seconds. The two halves also belong together. A check that
/// only presses Return on a hand opened form passes on a build where Create
/// does nothing at all, and a check that only presses it on a link raised one
/// passes on the same build for the opposite reason (L159). Sharing the app
/// makes the pairing structural rather than a convention between two files.
///
/// One method rather than two sharing a launch, deliberately: the halves run in
/// a fixed order and the second reads the store the first wrote. Two methods
/// would depend on XCTest's alphabetical ordering, which is a rule nothing
...
final class NewEventFormUITests: XCTestCase {
    ...
    func testReturnCommitsTheFormOnlyWhenItWasOpenedByHand() throws {
        ...
        // ── the other half of the same rule (#844) ───────────────────────────
        //
        // A link raises the form and brings PostRoll to the front at a moment
        // Dan did not choose, which on a cold launch is seconds after he
        // clicked and moved on. As the default action, Create then sits under
        // whatever he types next, and what it commits is an event whose whole
        // point is that it was reviewed first. It has happened: an event
        // reached the real store from a link with nobody pressing anything.
        let copy = try LaunchedApp.theRunningCopy()
        try fireLink(at: copy)
        usleep(3_000_000)
        print("NewEventFormUITests, link fired:\n\(AlertOnScreen.describe(app))")
        ...
        let raised = app.sheets.firstMatch
        XCTAssertTrue(raised.waitForExistence(timeout: 20),
        ...
        XCTAssertTrue(AlertOnScreen.words(in: app).contains { $0.contains("Hand check") },
                      "the form is not filled in from the link, so a Return here "
                      + "would commit nothing whatever the shortcut says. "
                      + AlertOnScreen.describe(app))

        // What the store held BEFORE, rather than asserting it is empty: the
        // half above put an event in it, and the question is whether this
        // Return ADDS one.
        ...
        usleep(3_000_000)
        XCTAssertEqual(storedNames() ?? [], before,
                       "Return committed an event Dan has never read, which is "
                       + "the whole of #844. The store went from \(before) to "
                       + "\(storedNames() ?? [])")
        XCTAssertTrue(raised.exists,
        ...
        // Create is broken passes the half above for the wrong reason (L159).
        let create = raised.buttons["Create Event"]
        XCTAssertTrue(create.exists, "there is no Create Event button to press")
        create.click()

        let deadline = Date().addingTimeInterval(10)
        while Date() < deadline, (storedNames() ?? []).count == before.count { usleep(200_000) }
        XCTAssertEqual(storedNames() ?? [], before + ["Hand check"],

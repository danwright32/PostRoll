...
# ── the closing crossfade (#335) ─────────────────────────────────────────────
#
# Every Tuesday reel ends by dissolving into the before/after graphic and
# holding on it. The reel's frames were checked and the graphic was checked as a
# still, and nothing measured the 1.5 seconds between them, which is the one
# window a still cannot show and the premise the whole frame-checking system was
# built on (#298).
#
# It is not a dissolve between two similar frames: the plate holds ONE print and
# the graphic holds three, and the caption moves from left-aligned below the
# print to centred above each strip, so two differently-placed captions are on
# screen at once at partial opacity.

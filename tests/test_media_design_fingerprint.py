...
def test_the_template_still_renders_the_design_its_version_claims(template, recorded):
    ...
    assert current == entry["fingerprint"], (
        ...
        f"  or the pixels moved and the design did not, which is an encode "
        f"setting rather than a redesign, so re-record the reference frame with "
        f"`make record-codec-change` and leave the version alone (#818).\n"
